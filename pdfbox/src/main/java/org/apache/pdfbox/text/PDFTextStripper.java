...
public class PDFTextStripper extends LegacyPDFStreamEngine {
     ...
     * example a PDF that has two columns like a newspaper, we want to extract the
     * first column and then the second column. In this example the PDF would have 2
     * beads(or articles), one for each column. The size of the charactersByArticle
     * would be 5, because not all text on the screen will fall into one of the
     * articles. The five divisions are shown below
     *
     * Text before first article first article text text between first article and
     * second article second article text text after second article
     *
     * Most PDFs won't have any beads, so charactersByArticle will contain a single
     * entry.
     */
     ...
     *
     * <p>
     * IMPORTANT: By default, text extraction is done in the same sequence as the
     * text in the PDF page content stream. PDF is a graphic format, not a text
     * format, and unlike HTML, it has no requirements that text one on page be
     * rendered in a certain order. The order is the one that was determined by the
     * software that created the PDF. To get text sorted from left to right and top
     * to botton, use {@link #setSortByPosition(boolean)}.
     * 
    ...
    /**
     * Gives you an array containing five Iterators: - Iterator<String> of all the
     * textual tokens in the given document. - Iterator<Float> containing the
     * x-coordinate of each textual token - Iterator<Float> containing the
     * x-coordinate of each textual token - Iterator<Float> containing the width of
     * each textual token - Iterator<Float> containing height of each textual token
     * They will be sorted on position depending on the state of sortByPosition.
     *
    ...
    public ListIterator<TextToken> getTextTokens(PDDocument doc, boolean skipHeaderAndFooter, float startY, float endY,
        ...
        for (Vector<TextPosition> textList = new Vector<TextPosition>(); pageIterator
            ...
            if (this.sortByPosition) {
                ...
                Collections.sort(currPageTextList, comparator);
            }

            textList.addAll(currPageTextList);
        }
    ...
    /**
     * This will take a PDDocument and write the text of that document to the print
     * writer.
     *
    ...
    /**
     * Start a new article, which is typically defined as a column on a single page
     * (also referred to as a bead). This assumes that the primary direction of text
     * is left to right. Default implementation is to do nothing. Subclasses may
    ...
    /**
     * Start a new article, which is typically defined as a column on a single page
     * (also referred to as a bead). Default implementation is to do nothing.
    ...
    /**
     * This will print the text of the processed page to "output". It will estimate,
     * based on the coordinates of the text, where newlines and word spacings should
     * be placed. The text will be sorted only if that feature was enabled.
     *
    ...
    protected void writePage() throws IOException {
        ...
        for (List<TextPosition> textList : charactersByArticle) {
            ...
            // add
            // spaces based on the distances between TextPositions. Historically, this was
            // done
            // based on the size of the space character provided by the font. In general,
            // this
            // worked but there were cases where it did not work. Calculating the average
            // character
            // width and using that as a metric works better in some cases but fails in some
            // cases
            // where the spacing worked. So we use both. NOTE: Adobe reader also fails on
            // some of
            // these examples.
            ...
            while (textIter.hasNext()) {
                ...
                if (lastPosition != null) {
                    ...
                    if (Float.compare(expectedStartOfNextWordX, EXPECTED_START_OF_NEXT_WORD_X_RESET_VALUE) != 0
                            ...
                            // only bother adding a word separator if the last character was not a word
                            // separator
                    ...
                    if (Math.abs(
                            position.getX() - lastPosition.getTextPosition().getX()) > (wordSpacing + deltaSpace)) {
                        maxYForLine = MAX_Y_FOR_LINE_RESET_VALUE;
                        maxHeightForLine = MAX_HEIGHT_FOR_LINE_RESET_VALUE;
                        minYTopForLine = MIN_Y_TOP_FOR_LINE_RESET_VALUE;
                    }
     ...
     * 
     * @throws IOException If there is a problem writing out the line separator to
     *                     the document.
     */
     ...
     * 
     * @throws IOException If there is a problem writing out the word separator to
     *                     the document.
     */
     ...
     * ignore the <code>textPositions</code> and just calls
    ...
    protected void processTextPosition(TextPosition text) {
        ...
        if (showCharacter) {
            // if we are showing the character then we need to determine which article it
            // belongs to
    ...
    /**
     * This is the page that the text extraction will start on. The pages start at
     * page 1. For example in a 5 page PDF document, if the start page is 1 then all
     * pages will be extracted. If the start page is 4 then pages 4 and 5 will be
    ...
    /**
     * This will get the last page that will be extracted. This is inclusive, for
     * example if a 5 page PDF an endPage value of 5 would extract the entire
     * document, an end page of 2 would extract pages 1 and 2. This defaults to
     ...
     * property is used if the line separator preference is not set explicitly using
     * this method.
     *
     ...
     * algorithm will output a space character if there is enough space between two
     * words. By default a space character is used. If you need and accurate count
     * of characters that are found in a PDF document then you might want to set the
     * word separator to the empty string.
     *
    ...
    /**
     * Character strings are grouped by articles. It is quite common that there will
     * only be a single article. This returns a List that contains List objects, the
     * inner lists will contain TextPosition objects.
     *
    ...
    /**
     * By default the text stripper will attempt to remove text that overlapps each
     * other. Word paints the same character several times in order to make it look
     * bold. By setting this to false all text will be extracted, which means that
     * certain sections will be duplicated, but better performance will be noticed.
     *
     * @param suppressDuplicateOverlappingTextValue The
    ...
    /**
     * Set if the text stripper should group the text output by a list of beads. The
    ...
    /**
     * Get the bookmark where text extraction should start, inclusive. Default is
     * null.
     *
    ...
    /**
     * There will some additional text formatting be added if addMoreFormatting is
     ...
     * estimate where spaces in text should be added. Note that the default value
     * for this has been determined from trial and error.
     * 
     ...
     * spaces in text should be added. Note that the default value for this has been
     * determined from trial and error. Setting this value larger will reduce the
    ...
    /**
     * Get the current character width-based tolerance value that is being used to
     * estimate where spaces in text should be added. Note that the default value
     * for this has been determined from trial and error.
     * 
     * @return The current tolerance / scaling factor
    ...
    /**
     * Set the character width-based tolerance value that is used to estimate where
     * spaces in text should be added. Note that the default value for this has been
     * determined from trial and error. Setting this value larger will reduce the
     * number of spaces added.
     * 
    ...
    /**
     * sets the multiple of whitespace character widths for the current text which
     * the current line start can be indented from the previous line start beyond
     * which the current line start is considered to be a paragraph start. The
    ...
    /**
     * sets the minimum whitespace, as a multiple of the max height of the current
     * characters beyond which the current line start is considered to be a
     * paragraph start. The default value is 2.5.
     *
     * @param dropThresholdValue the character height multiple for max allowed
     *                           whitespace between lines in the same paragraph.
     */
     ...
     * position and the last text position that followed a line separator to decide
     * if the gap represents a paragraph separation. This should <i>only</i> be
     * called for consecutive text positions that first pass the line separation
     * test.
     * <p>
     * This base implementation tests to see if the lastLineStartPosition is null OR
     * if the current vertical position has dropped below the last text vertical
     * position by at least 2.5 times the current text height OR if the current
     * horizontal position is indented by at least 2 times the current width of a
     * space character.
     * </p>
     * <p>
     ...
     * </p>
     *
     * @param position              the current text position. This may have its
     *                              isParagraphStart or isHangingIndent flags set
     *                              upon return.
     * @param lastPosition          the previous text position (should not be null).
     * @param lastLineStartPosition the last text position that followed a line
     *                              separator, or null.
     * @param maxHeightForLine      max height for text positions since
     *                              lasLineStartPosition.
     */
    ...
    /**
     * returns the list item Pattern object that matches the text at the specified
     * PositionWrapper or null if the text does not match such a pattern. The list
     * of Patterns tested against is given by the {@link #getListItemPatterns()}
     * method. To add to the list, simply override that method (if sub-classing) or
     * explicitly supply your own list using {@link #setListItemPatterns(List)}.
     * 
     * @param pw position
    ...
    /**
     * returns a list of regular expression Patterns representing different common
     * list item formats. For example numbered items of form:
     * <ol>
     * <li>some text</li>
     * <li>more text</li>
     * </ol>
     ...
     * <li>more text</li>
     * </ul>
     * etc., all begin with some character pattern. The pattern "\\d+\." (matches
     * "1.", "2.", ...) or "\[\\d+\]" (matches "[1]", "[2]", ...).
     * <p>
    ...
    /**
     * iterates over the specified list of Patterns until it finds one that matches
     * the specified string. Then returns the Pattern.
     * <p>
     * Order of the supplied list of patterns is important as most common patterns
     * should come first. Patterns should be strict in general, and all will be used
     ...
     * implementation stands and falls with the given word. If the word is a full
     * line, the results will be the best. If the word contains of single words or
     * characters, the order of the characters in a word or words in a line may
     * wrong, due to RTL and LTR marks and characters!
     * 
     * Based on
    ...
    /**
     * Normalize certain Unicode characters. For example, convert the single "fi"
     * ligature to "f" and "i". Also normalises Arabic and Hebrew presentation
     * forms.
     *
     ...
     * Note that the number of entries in that list may differ from the number of
     * characters in the string due to normalization.
     *
     * @author Axel Dörfler
     */
